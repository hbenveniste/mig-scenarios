...
sspall = CSV.File(joinpath(@__DIR__, "../results/sspall_6.csv")) |> DataFrame
...
pop_world_mig = combine(groupby(ginibtw_mig, [:scen, :period]), d -> sum(d.pop_mig))
rename!(pop_world_mig, :x1 => :pop_world_mig)
cum_pop=[]
...
pop_world_nomig = combine(groupby(ginibtw_nomig, [:scen, :period]), d -> sum(d.pop_nomig))
rename!(pop_world_nomig, :x1 => :pop_world_nomig)
cum_pop=[]
for i in 0:length(periods)*length(ssps)-1
    cp = []
    for c in 1:length(countries)
...
gdp_world_mig = combine(groupby(ginibtw_mig, [:scen, :period]), d -> sum(d.gdp_mig))
rename!(gdp_world_mig, :x1 => :gdp_world_mig)
cum_gdp=[]
for i in 0:length(periods)*length(ssps)-1
    cp = []
    for c in 1:length(countries)
...
gdp_world_nomig = combine(groupby(ginibtw_nomig, [:scen, :period]), d -> sum(d.gdp_nomig))
rename!(gdp_world_nomig, :x1 => :gdp_world_nomig)
cum_gdp=[]
for i in 0:length(periods)*length(ssps)-1
    cp = []
    for c in 1:length(countries)
...
) |> save(joinpath(@__DIR__, "../results/gini_btw/", "gini_world_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/gini_btw/", "ginirel_world_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/gini_btw/", "lorenz_2065_world_6.png"))


####################################### Write output files with results ############################################################################
CSV.write(joinpath(@__DIR__, "../results/gini_btw_6.csv"), gini_world)