...
mig0_ssp4 = CSV.File(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/mig0_SSP4.csv")) |> DataFrame   # file too large for Github
ssp4 = CSV.File(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP4.csv")) |> DataFrame    # file too large for Github
mig0_ssp5 = CSV.File(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/mig0_SSP5.csv")) |> DataFrame    # file too large for Github
ssp5 = CSV.File(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP5.csv")) |> DataFrame     # file too large for Github
...
gdp_oecd = load(joinpath(@__DIR__, "../data/gdp_sspdb/gdp_oecd.xlsx"), "data!A1:X921") |> DataFrame
...
# Join population and GDP data: convert into iso3c country codes
...
migflow[!,:move_prelim] = exp(beta[ireg,:b0]) .* migflow[:,:pop_orig].^beta[ireg,:b1] .* migflow[:,:pop_dest].^beta[ireg,:b2] .* migflow[:,:ypc_orig].^beta[ireg,:b4] .* migflow[:,:ypc_dest].^beta[ireg,:b5] .* migflow[:,:distance].^beta[ireg,:b7] .* map(x -> exp(beta[ireg,:b8] * x), migflow[:,:remshare]) .* map(x -> exp(beta[ireg,:b9] * x), migflow[:,:remcost]) .* map(x -> exp(beta[ireg,:b10] * x), migflow[:,:comofflang])
...
movein = combine(groupby(migflow, [:destination, :period, :scen]), d -> (move_in_prelim = sum(d.move_prelim), inmig_dest_ = mean(d.inmig_dest)))
migflow = innerjoin(migflow, movein, on = [:destination, :period, :scen])
select!(migflow, Not(:inmig_dest_))
...
for i in 1:size(agegroup,1) ; if agegroup[i,:inmig] == 0 && agegroup[i,:inmig_sum] == 0 ; agegroup[i,:share] = 0 end end
...
migflow_all[!,:stock0] = [migflow_all[i, :move_inmigbase] * migflow_all[i,:share] * 5 for i in 1:size(migflow_all,1)] 
...
indmissing = findall([typeof(migstock[i,:stock]) != Float64 for i in 1:size(migstock, 1)])
...
ccode = load(joinpath(@__DIR__,"../data/rem_wb/GDPpercap2017.xlsx"), "Data!A1:E218") |> DataFrame
...
migstock[!,:stock_by_age] = migstock[!,:stock] .* migstock[!,:meanshare]
...
indrem = []
for i in 1:size(stock_by_age,1)
...
remittances = migflow[:,[:scen, :origin, :destination, :period, :ypc_orig, :ypc_dest, :remcost, :exp_residual]]

remittances = DataFrame(scen = migflow[!,:scen], origin = migflow[!,:origin], destination = migflow[!,:destination], period = migflow[!,:period], migstock = migrantstock[!,:migrantstock], ypc_orig = migflow[!,:ypc_orig], ypc_dest = migflow[!,:ypc_dest])
rho = CSV.File(joinpath(@__DIR__, "../data/rem_wb/rho.csv")) |> DataFrame
phi = CSV.File(joinpath(@__DIR__,"../data/rem_wb/phi.csv")) |> DataFrame
...
remittances[!,:remittances] = [remittances[i,:migrantstock] * max(mean([remittances[i,:ypc_orig], remittances[i,:ypc_dest]]), remittances[i,:ypc_orig]) * remittances[i,:rho] * (1 - remittances[i,:phi]) for i in 1:size(remittances,1)]
...
# For 3 countries, |rem| > GDP at some periods: Liberia, Sierra Leone, Central African Republic
...
) |> save(joinpath(@__DIR__, "../results/pop_all/", "immigshare_world_bar_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/gdp_all/", "gdp_world_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/ypc_all/", "ypc_world_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/ypc_rel/", "ypcrel_world_6.png"))
...
) |> save(joinpath(@__DIR__, "../results/pop_all/", "pop_MEX_6.png"))
...
for s in ssps
    ...
    ) |> save(joinpath(@__DIR__, "../results/world_maps/", string("popdiff_", s, "_6.png")))
end
...
for s in ssps
    ...
    ) |> save(joinpath(@__DIR__, "../results/world_maps/", string("gdpdiff_corr_", s, "_6.png")))
end
...
for s in ssps
    ...
    ) |> save(joinpath(@__DIR__, "../results/world_maps/", string("ypcdiff_corr_", s, "_6.png")))
end
...
for r in unique(regions[!,:worldregion])
    ypc_reg |> @filter(_.worldregion == r) |> @vlplot(
        ...
        color = {"scen:n", scale={scheme=:category10}, legend={titleFontSize=16, symbolSize=40, labelFontSize=16}}, 
        shape = {"ypc_type:o", scale={range=["circle","triangle-up"]}, legend={titleFontSize=16, symbolSize=40, labelFontSize=16}}
    ) |> save(joinpath(@__DIR__, "../results/ypc_reg/", string("ypc_", r, "_6.pdf")))
end
...
for r in unique(regions[!,:worldregion])
    sspall_reg |> @filter(_.worldregion == r) |> @vlplot(
        ...
        title = string(r), 
        color = {"scen:n", scale={scheme=:category10}, legend=nothing}
    ) |> save(joinpath(@__DIR__, "../results/ypc_reg/", string("ypcrel_", r, "_6.pdf")))
end
...
for d in [2050, 2095]
    for s in ssps
        ...
        ) |> save(joinpath(@__DIR__, "../results/pop_reg/", string("migflow_reg_", d,"_", s, "_6.png")))
        migflow_reg |> @filter(_.period == d && _.scen == s) |> @vlplot(
            :rect, y=:or_reg, x=:dest_reg, color={"share_or_reg:q", scale={domain=[0,0.0001], scheme=:goldred}},title = string(s, ", ", d)
        ) |> save(joinpath(@__DIR__, "../results/pop_reg/", string("migflow_reg_share_or_", d,"_", s, "_6.png")))
        migflow_reg |> @filter(_.period == d && _.scen == s) |> @vlplot(
            :rect, y=:or_reg, x=:dest_reg, color={"share_dest_reg:q", scale={domain=[0,0.0001], scheme=:goldred}},title = string(s, ", ", d)
        ) |> save(joinpath(@__DIR__, "../results/pop_reg/", string("migflow_reg_share_dest_", d,"_", s, "_6.png")))
    end
end
...
for d in [2050, 2095]
    for s in ssps
        ...
        ) |> save(joinpath(@__DIR__, "../results/ypc_reg/", string("remittances_reg_", d,"_", s, "_6.png")))
        remittances_reg |> @filter(_.period == d && _.scen == s) |> @vlplot(
            :rect, y=:or_reg, x=:dest_reg, color={"remshare_or_reg:q", scale={domain=[0,0.01]}},title = string(s, ", ", d)
        ) |> save(joinpath(@__DIR__, "../results/ypc_reg/", string("remittances_reg_share_or_", d,"_", s, "_6.png")))
        remittances_reg |> @filter(_.period == d && _.scen == s) |> @vlplot(
            :rect, y=:or_reg, x=:dest_reg, color={"remshare_dest_reg:q", scale={domain=[0,0.01]}},title = string(s, ", ", d)
        ) |> save(joinpath(@__DIR__, "../results/ypc_reg/", string("remittances_reg_share_dest_", d,"_", s, "_6.png")))
    end
end
...
CSV.write(joinpath(@__DIR__, "../results/sspall_6.csv"), sspall)
CSV.write(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/results/migflow_6.csv"), migflow)
CSV.write(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/results/remittances_6.csv"), remittances)
